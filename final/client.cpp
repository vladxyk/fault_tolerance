...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if (connect(socket_desc[i], (struct sockaddr *)&server[i], sizeof(server[i])) == -1)
        {
            // puts("connect error");
            ...
            main_port++;
            socket_desc.push_back(socket(AF_INET, SOCK_STREAM, 0));
            server[i + 1].sin_addr.s_addr = INADDR_ANY;
            server[i + 1].sin_family = AF_INET;
            server[i + 1].sin_port = htons(main_port + 1);
            port = main_port + 1;
            continue;
        }
        else
